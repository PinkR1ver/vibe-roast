...
AI hashtags use a paired bilingual schema (`en`, `zh`, `kind`, and a short semantic `meaning`) so both languages must express the same joke. Chinese is a localization of the intended meaning rather than a surface-form translation. Fixed personality labels reuse the deterministic bilingual profile titles, while proper nouns and meaningful acronyms may remain unchanged. Evidence schema 2 records recurring same-prompt domain clusters without including raw prompt text, filters app-owned context, identifiers, status prose, and operational boilerplate, then exposes multiple candidates so the writer can favor semantic distinctiveness over raw frequency. Writer prompt version 6 deliberately treats tags as comedy stage names rather than analytics labels: two repeated topic prompts or one distinctive recurring concept can now seed a fictional alter ego, five tags have no rigid role order, and slang, puns, mock jobs, props, and subculture references are encouraged. Factual roast claims remain evidence-bound, but a repeated game + ammunition + price-market prompt-world may freely become `ArmsDealer`/`武器贩子` as an obviously fictional handle rather than a claim about the user's occupation.

## README ensemble hero

The README opens with a 1983×793 WebP ensemble banner containing all sixteen personality figures exactly once. The generated low-poly stage preserves the established character palette and prop language, while deterministic typography uses the same Great Vibes face as the in-app loading/header wordmark. The wordmark, personality line, and `16 TYPES / NO ALIBIS` now sit directly in a feathered extension of the stage light with no card, border, or button container, so the brand reads as part of the scene. The optimized repository asset is approximately 216 KB and replaces the former single-Builder portrait without changing the semantic page heading or badge/navigation block.

The live-result screenshot was recaptured from the current dark-mode application at 1274×717. It now demonstrates the token-only KPI contract, Agent icons, provider/model fields, the Builder type, and topic-aware poker hashtags instead of the retired Prompt-count `TOP CATEGORY` card and category-frequency tags.
