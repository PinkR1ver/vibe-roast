...
- The local server keeps an HttpOnly SameSite cookie and persists its session outside the project at `~/.vibe-roast/github-auth.json` with `0600` permissions.
...
- A direct OAuth mode using `VIBE_ROAST_GITHUB_CLIENT_SECRET` remains available for local development only.
