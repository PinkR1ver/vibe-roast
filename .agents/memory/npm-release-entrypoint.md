...
- `tokentracker-cli` is a production dependency. The first `vibe-roast` execution visibly initializes its collectors/hooks and later executions sync its local queue before serving; npm package installation itself remains side-effect free.
...
- v0.98.3 bundles `tokentracker-cli`, initializes/syncs TokenTracker before reports, keeps Activity in Token mode, and adds an anonymized Roast Result screenshot to the README.
