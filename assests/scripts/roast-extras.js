...
/** @param {{ limit?: number, locale?: "en"|"zh" }|number} [opts] */
export function buildHashtags(vibe, categories = {}, opts = {}) {
  ...
  if (ARCHETYPE_TAGS[archId]) push(pickLocale(ARCHETYPE_TAGS[archId], zh));
  else if (arch.title) push(zh ? arch.title : pascalCase(arch.title));
  if (arch.code && CODE_TAGS[arch.code]) push(pickLocale(CODE_TAGS[arch.code], zh));
  ...
  if (hookSrc) {
    ...
    if (hookWord && hookWord.length > 1) push(zh ? hookWord : pascalCase(hookWord));
  }
